function Aircraft_Analyzer_Program()
clc;
devam='E';
k=0;
    fprintf('\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tWelcome to the AIRCRAFT ANALYZATION PROGRAM \n\n\n');
%     pause(2);
    fprintf('You can calculate Aircraft performance at constant altitude and velocity which you choosed.\n\n\n');
    beep;
    fprintf(' While using the program please READ ALL OF DESCRIPTIONS , WARNINGS and ERROR MESSAGES \n\n');
%     pause(6);
    sayac=0;
while devam=='E' | devam=='Y'
    ...
    dongudevam=0;
    i=0;
    kontrol3=0;kontrol4=0;kontrol5=0;kontrol6=0;kontrol7=0;kontrol9=0;kont88=0;continue1=0;
    sayac=sayac+1;
    %%
    while i<=1
        ...
        if istek=='y' & isempty(str2num(istek))==1
            ...
            while continue1=='y'
                ...
                [MainMatrix numbers planenumbers]=NewPlaneSave(Matrix,MainMatrix,ucakadiyaz,numbers);
    ...
    while dongudevam==0
        ...
        while kont88==0
            ...
            %%
            while count43==0
                planename=input('Please enter a plane number that you want to delete: ', 's');
                if isempty(planename)~=1 & isempty(str2num(planename))~=1 & max(planenumbers)>=str2num(planename)
                    planename=str2num(planename);
                    [numbers MainMatrix]=Delete_Plane(planename,numbers,MainMatrix,planenumbers);
        ...
        while  count87==0
            ...
            if isempty(str2num(planename))~=1 & str2num(planename)<=max(planenumbers)
                count87=1;
                planename=str2num(planename);
                [numbers planename]=Identifying_Plane(numbers,planename,planenumbers);
        ...
        fprintf('\nAircraft velocity that you choosed: %gm/s\n',V1);
        fprintf('\nAltitude that you choosed: %gm\n',HeightAltitude);
        j=planename;
        aircraftname=MainMatrix(j+1,2);
        format bank;%2 ondalýk
        ...
        fprintf('\nHeight(m): %1.2f\tTemperature(Kelvin): %1.2f\tPressure(Pa): %1.2f\t Density(kg/m^3): %1.2f\n\nMax Velocity(m/s):%1.2f\nAspect ratio: %1.2f(non-unit) \nCD-Ý:%1.5f(non-unit) \nCD: %1.2f(non-unit) \nLift: %1.2f(newton)\nDrag: %1.2f(newton) \nDynamicPressure: %1.2f(kg/m*sec^2)\nInduced Drag: %1.2f(newton)\nPower Requirement: %1.2f(watt)\nThrust Reqirement: %1.2f(newton)\n *****************//////***********\n\n',HeightAltitude,temperature,press1,Density,max_velocity,AR,CDi,CD,lift,drag,dynamicpressure,induceddrag,PoweReqr,ThrustReqr);
        while kontrol5==0;
        ...
        if cikti=='Y'
            count5=0;
            while count5==0
                how=input('Which format do you want to output ? (for txt -->1 / for xlsx--->2 / for both ---->3 (absolutely ''NAME THAT YOU ENTERED''.mat file will save)): ','s');
                if size(how,1)==0
                    msgbox('There is no input argument!', 'ERROR!','error');
                    beep;
                    count5=0;
            ...
            switch how
                ...
                case 2
                    ...
                    [ffilename filename]=Print_XLSX(max_velocity,AR,CDi,CD,lift,drag,dynamicpressure,induceddrag,PoweReqr,ThrustReqr,filename);
                    kontrol6=1;
                    while kontrol9==0;
                ...
                case 3
                    ...
                    [ffilename filename]=Print_TXT(filename,max_velocity,AR,CDi,CD,lift,drag,dynamicpressure,induceddrag,PoweReqr,ThrustReqr);
                    kont6=0;
                    while kont6==0
        ...
        count_mat=0;
        while count_mat==0
            filename_mat=input('Please enter a name for have to save the .mat file: ','s');
            if size(filename_mat,1)==0
                msgbox('There is no input argument!', 'ERROR!','error');
                beep;
                count_mat=0;
            else
        ...
        Print_Mat(filename_mat,MainMatrix);
        ...
        if draw=='y'
            ...
            if graph==1
                Graph_CL_CD(max_velocity,weight,wingarea,Density,oswald,AR,CD0_1,aircraftname,HeightAltitude);
            elseif graph==2
                Graph_Dynamic(max_velocity,Density,aircraftname,HeightAltitude);
            elseif graph==3
                Graph_Both(max_velocity,weight,wingarea,Density,oswald,AR,CD0_1,aircraftname,HeightAltitude)
            else
                disp('---------*************--------------');
            end
        end
        ...
        if devam=='N'
            ...
            pause(4)
            clc;
            fprintf('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n \t\t\t\t\t\t\t\t\t\t\t\t\t\t  All Rights Reserved By Tevfik Erkin\n');
        end
    end
end
%%